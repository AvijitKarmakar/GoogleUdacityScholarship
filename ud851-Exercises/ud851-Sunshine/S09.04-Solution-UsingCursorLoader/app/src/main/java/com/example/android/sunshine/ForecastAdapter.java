...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        String date = SunshineDateUtils.getFriendlyDateString(mContext, mCursor.getLong(
                MainActivity.INDEX_WEATHER_DATE), false);
        ...
        String highLowTemp = SunshineWeatherUtils.formatHighLows(mContext,
                mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP),
                mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP));
        String weatherForThisDay = date + " - " + description + " - " + highLowTemp;
